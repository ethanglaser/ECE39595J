...
\   * return -1 if this < a, 0 if this equals a, 1 if this > a
   ...
   public int compareTo(ComparableArray a){
      int status = 0;
      if (a.ary.length == this.ary.length){
         for (int i=0; i < this.ary.length; i++){
            if (this.ary[i] > a.ary[i] && status == 0){
               status = 1;
            }
            else if (this.ary[i] < a.ary[i] && status == 0){
               status = -1;
            }
         }
      }
      else if(a.ary.length > this.ary.length){
         for (int i=0; i < this.ary.length; i++){
            if (this.ary[i] > a.ary[i] && status == 0){
               status = 1;
            }
         ...
         if (status == 0) {
            for (int i = this.ary.length; i < a.ary.length; i++) {
               if (a.ary[i] != 0) {
                  status = -1;
               }
            }
         ...
         if (status == 0) {
            for (int i = a.ary.length; i < this.ary.length; i++) {
               if (this.ary[i] != 0) {
                  status = 1;
               }
            }
